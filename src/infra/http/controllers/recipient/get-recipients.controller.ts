...
const recipientResponseSchema = z.object({
  ...
  cpf: z
    .string()
    .describe("The recipient's CPF in formatted form (e.g., 123.456.789-01)"),
  email: z.string().email().describe("The recipient's email address"),
  createdAt: z
    .string()
    .describe('The creation timestamp of the recipient record'),
  updatedAt: z
    .string()
    .describe('The last update timestamp of the recipient record'),
})
